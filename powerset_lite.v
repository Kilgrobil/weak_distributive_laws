...
Definition im [X] [Y] (f : X -> Y) (a : P X) (y : Y) := exists (x : X), a x /\ f x = y.
Definition etaP X (x : X) (x' : X) := x = x'.
Definition muP X (t : P (P X)) (x : X) := exists (s : P X), s x /\ t s.

(** In any topos, extensionality hold. *)
...
Lemma image_of_full_is_singleton : im Prop_into_terminal full = etaP terminal elt.
Proof.
apply ext. unfold im,full,etaP,Prop_into_terminal. intro. destruct x. split ; intro.
  - reflexivity.
  - exists True. split ; trivial.
Qed.

(** The unit [etaP] is nearly cartesian if and only if the topos is degenerate. *)
Theorem eta_nearly_cartesian : (forall X Y (f : X -> Y) (s : P X) (y : Y), 
  im f s = etaP Y y -> exists (x : X), etaP X x = s /\ f x = y) <-> (True = False).
Proof.
split ; intros.
  - specialize (H Prop terminal Prop_into_terminal).
    pose proof H full elt image_of_full_is_singleton.
    destruct H0,H0. assert (etaP Prop x True <-> True) by (rewrite H0 ; intuition).
    assert (etaP Prop x False <-> True) by (rewrite H0 ; intuition). unfold etaP in H2,H3.
    assert (x = True) by (apply H2 ; trivial).
    assert (x = False) by (apply H3 ; trivial). rewrite H4 in H5. assumption.
...
Proof.
intros. apply ext. unfold preim,inter,im. intro y. split ; intros ; destruct H,H,H.
  - rewrite H0 in H1. split ; try assumption. exists x. split ; assumption.
  - rewrite <- H1 in H0. exists x. auto.
Qed.

Lemma subset_in_union : forall X (t : P (P X)) (s : P X), t s -> muP X t ∩ s = s.
Proof.
...
Theorem mu_nearly_cartesian : forall X Y (f : X -> Y) (s : P X) (t' : P (P Y)), im f s = muP Y t' ->
  exists (t : P (P X)), muP X t = s /\ im (im f) t = t'.
Proof.
intros. exists (im (fun s' => s ∩ (preim f s')) t'). split ; apply ext.
  - intro. unfold muP,im. split ; intro.
    + destruct H0 as (s0,H0),H0,H1 as (s',H1),H1. rewrite <- H2 in H0. destruct H0. assumption.
    + assert ((im f s) (f x)) by (unfold im ; exists x ; split ; try assumption ; reflexivity).
      rewrite H in H1. destruct H1 as (s', H1), H1. exists (s ∩ (preim f s')). split.
      * split ; assumption.
      * exists s'. auto.
  - intro s'. unfold im. split ; intro.
    + destruct H0 as (s0,H0),H0,H0 as (s'0,H0),H0. pose proof frobenius f s s'0. rewrite H2,H in H3.
      rewrite subset_in_union in H3 ; try assumption. unfold im in H3. rewrite <- H1, H3. assumption.
    + exists (s ∩ (preim f s')). split.
      * exists s'. auto.
      * pose proof frobenius f s s'. rewrite H in H1. rewrite subset_in_union in H1 ; assumption.
Qed.

(** *                                 Monotone weak distributive law                             *)

Definition relation [X] [Y] (R : P(X * Y)) := { t : X * Y | R t}.
Definition proj1 [X] [Y] (R : P(X * Y)) (t : relation R) := fst (proj1_sig t).
Definition proj2 [X] [Y] (R : P(X * Y)) (t : relation R) := snd (proj1_sig t).
Definition Pext [X] [Y] (R : P(X * Y)) (u : P X * P Y) := 
    exists (t : P (relation R)), im (proj1 R) t = fst u /\ im (proj2 R) t = snd u.

Lemma pair_in_relation [X] [Y] (R : P(X * Y)) : forall r : relation R, R (proj1 R r, proj2 R r).
Proof.
intro. destruct r as (t,r0). destruct t. unfold proj1,proj2. simpl. assumption.
Qed.

(** The relational extension is given by the Egli-Milner formula. *)
Proposition Egli_Milner [X] [Y] (R : P(X * Y)) : forall (u : P X * P Y), Pext R u <->
...
intro. destruct u as (s,s'). unfold Pext. simpl. split ; intro.
  - destruct H as (t,H),H. rewrite <- H, <- H0. unfold im. split.
    + intros. destruct H1 as (r,H1). destruct H1. exists (proj2 R r). split.
      * exists r. auto.
      * rewrite <- H2. apply pair_in_relation.
    + intros. destruct H1 as (r,H1). destruct H1. exists (proj1 R r). split.
      * exists r. auto.
      * rewrite <- H2. apply pair_in_relation.
  - destruct H. pose (fun (t0 : relation R) => s (fst (proj1_sig t0)) /\ s' (snd (proj1_sig t0))) as t.
    exists t. split.
    + apply ext. intro x. split ; intro.
      * unfold im in H1. destruct H1 as (r,H1),H1. destruct r as (r0,H3).
        unfold t in H1. unfold proj1 in H2. simpl in *. destruct H1. rewrite H2 in H1. assumption.
      * specialize (H x H1). destruct H as (y,H),H. unfold im. exists (exist R (x,y) H2).
        unfold t. simpl. split ; try split ; assumption.
    + apply ext. intro y. split ; intro.
      * unfold im in H1. destruct H1 as (r,H1),H1. destruct r as (r0,H3). unfold t in H1.
        unfold proj2 in H2. simpl in *. destruct H1. rewrite H2 in H4. assumption.
      * specialize (H0 y H1). destruct H0 as (x,H0),H0. unfold im. exists (exist R (x,y) H2).
        unfold t. simpl. split ; try split ; assumption.
Qed.
...
Theorem monotone_weak_dlaw X : forall (t : P (P X)) (s : P X), lambda_m X t s <-> 
    (forall x : X, s x -> muP X t x) /\ (forall u : (P X), t u -> exists (x : X), u x /\ s x).
Proof.
  intros. unfold lambda_m. unfold muP. split ; intro.
  - apply Egli_Milner in H. unfold fst,snd in H. destruct H. split.
    + intros. specialize (H0 x H1). destruct H0 as (s0,H0),H0. exists s0. split ; assumption.
    + intros. specialize (H u H1). destruct H as (x,H),H. exists x. split ; assumption.
  - apply Egli_Milner. unfold fst,snd. destruct H. split.
    + intro u. intro. specialize (H0 u H1). destruct H0 as (x,H0). exists x. destruct H0. split ; assumption.
    + intro x. intro. specialize (H x H1). destruct H as (u,H). exists u. destruct H. split ; assumption.
Qed.

(** *                                    Distributive law                                        *)

(** We prove that if there is a natural transformation [lambda : PP -> PP] satisyfing both unit axioms,
then [True = False]. This is the formalisation of a proof by Klin and Salamanca in 
...
Proof. intros. unfold subset,im,preim. intros. apply H. exists x. auto. Qed.
...
Lemma im_two [X] [Y] (f : X -> Y) : forall (x y : X), (im f) (two x y) = two (f x) (f y).
Proof.
...
(** "Assume, towards a contradiction, that there is such a distributive law." 
Using the following axioms, we will be able to derive that True = False *)
Axiom lambda : forall X, P (P X) -> P (P X).
Axiom naturality : forall X Y (f : X -> Y) (t : P (P X)), 
      (im (im f)) (lambda X t) = lambda Y ((im (im f)) t).
Axiom unit_1 : forall X (s : P X), (lambda X) (etaP (P X) s) = im (etaP X) s.
Axiom unit_2 : forall X (s : P X), (lambda X) (im (etaP X) s) = etaP (P X) s.

(** "Consider sets:" (in a topos these are finite coproducts) *)
Inductive A := a | b | c | d.
Inductive U := u | v.

Lemma inter_two (x y z : A) : ~(y = z) -> (two x y) ∩ (two x z) = etaP A x.
Proof.
...
(** "Analyse how the three naturality squares for [f], [g] and [h] act on [t].
Recall that [im] acts on functions by taking direct images, so in particular:" *)
...
Proof. unfold t. rewrite im_two. rewrite Pg_ab,Pg_cd. apply idempotence. Qed.

Lemma PPh_t : (im (im h)) t = etaP (P U) (two u v).
Proof. unfold t. rewrite im_two. rewrite Ph_ab,Ph_cd. apply idempotence. Qed.

(** "Hence, by a unit law for [lambda] and naturality squares for [g] and [h] we obtain:"*)
Lemma PPg_lambdaA_t : (im (im g)) (lambda A t) = two (etaP U u) (etaP U v).
Proof. rewrite naturality. rewrite PPg_t. rewrite unit_1. apply im_two. Qed.

Lemma PPh_lambdaA_t : (im (im h)) (lambda A t) = two (etaP U u) (etaP U v).
Proof. rewrite naturality. rewrite PPh_t. rewrite unit_1. apply im_two. Qed.
...
(** "and:" *)
Lemma Pg_s : forall (s : P A), lambda A t s -> (two (etaP U u) (etaP U v)) (im g s).
Proof.
intros. rewrite <- PPg_lambdaA_t. unfold im. exists s. intuition.
Qed.

Lemma Ph_s : forall (s : P A), lambda A t s -> (two (etaP U u) (etaP U v)) (im h s).
Proof.
...
Proof.
intros. split.
  - pose proof Pg_s s H. destruct H0 ; apply subset_refl in H0 ; apply pres_preim in H0.
    + rewrite preim_g_u in H0. auto.
    + rewrite preim_g_v in H0. auto.
  - pose proof Ph_s s H. destruct H0 ; apply subset_refl in H0 ; apply pres_preim in H0.
...
Lemma lambdaA_t_bis : forall (s : P A), lambda A t s -> (exists (x : A), s ⊆ etaP A x).
Proof.
intros. apply lambdaA_t in H. destruct H,H,H0.
  - exists a. rewrite <- inter_two with (y := c) (z := d) ; try discriminate.
    apply subset_inter ; assumption.
  - rewrite symmetry in H,H0. exists c. rewrite <- inter_two with (y := a) (z := b) ;
    try discriminate. apply subset_inter ; assumption.
  - rewrite symmetry in H,H0. exists d. rewrite <- inter_two with (y := a) (z := b) ;
    try discriminate. apply subset_inter ; assumption.
  - exists b. rewrite <- inter_two with (y := c) (z := d) ; try discriminate.
    apply subset_inter ; assumption.
Qed.

(** "Now let us come back to the function [f]. By naturality of [two] we get:" *)
Lemma PPf_t : (im (im f)) t = two (etaP U u) (etaP U v).
Proof. unfold t. rewrite im_two. rewrite Pf_ab,Pf_cd. reflexivity. Qed.

(** "Hence, by the naturality square for [f] and by a unit law for [lambda]:" *)
Lemma PPf_lambdaA_t : im (im f) (lambda A t) = etaP (P U) (two u v).
Proof.
rewrite naturality. rewrite PPf_t. rewrite <- unit_2. rewrite im_two. reflexivity.
Qed.

(** "This means that:" *)
Lemma Pf_expression : forall (s : P A), (lambda A t s) -> (im f) s = two u v.
Proof.
intros. pose proof PPf_lambdaA_t.
assert (im (im f) (lambda A t) (im f s) <-> etaP (P U) (two u v) (im f s))
by (rewrite H0 ; intuition). unfold etaP in H1. symmetry. apply H1.
exists s. auto.
Qed.
...
Proof.
pose proof lambdaA_t_nonempty. destruct H as (s,H). pose proof Pf_expression s H.
